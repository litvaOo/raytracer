...
ray_color :: proc(ray: ^Ray, world: [dynamic]Hittable, depth: u32, alloc: runtime.Allocator) -> Vector {
  if depth <= 0 {
    return Vector{0, 0, 0}
  }
  hit_rec := HitRecord{}
  ...
  if hittable_list_hit(world, ray, 0.001, math.F64_MAX, &hit_rec) == true {
    ...
    switch &mat in hit_rec.material {
      case Metal:
        if metal_scatter(ray, &hit_rec, &attenuation, &scattered, &mat, alloc) == true
          { return attenuation * ray_color(&scattered, world, depth-1, alloc) }
        return Vector{0,0,0}
      case Lambertian:
        if lambertian_scatter(ray, &hit_rec, &attenuation, &scattered, &mat, alloc) == true
          { return attenuation * ray_color(&scattered, world, depth-1, alloc) }
        return Vector{0,0,0}
      case Dielectric:
        if dielectric_scatter(ray, &hit_rec, &attenuation, &scattered, &mat, alloc) == true
          { 
            return attenuation * ray_color(&scattered, world, depth-1, alloc) }
        return Vector{0,0,0}
    }
