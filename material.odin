...
lambertian_scatter :: proc (ray_in: ^Ray, hit_rec: ^HitRecord, attenuation: ^Vector, scattered: ^Ray, material: ^Lambertian, alloc: runtime.Allocator) -> bool{
  scatter_direction := new(Vector, alloc)
...
metal_scatter :: proc (ray_in: ^Ray, hit_rec: ^HitRecord, attenuation: ^Vector, scattered: ^Ray, material: ^Metal, alloc: runtime.Allocator) -> bool {
  reflected := new(Vector, alloc)
...
dielectric_scatter :: proc(ray_in: ^Ray, hit_rec: ^HitRecord, attenuation: ^Vector, scattered: ^Ray, material: ^Dielectric, alloc: runtime.Allocator) -> bool {
  attenuation^ = Vector{1.0, 1.0, 1.0}
  ri := hit_rec.front_face ? (1.0/material.refraction_index) : material.refraction_index
  unit_direction := new(Vector, alloc)
